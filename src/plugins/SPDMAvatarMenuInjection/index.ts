...
export default class SPDMAvatarInjection implements SpotifyPlugin {
    ...
    public load() {
        ...
        SpotifyEventListener.on('avatar-menu-opened', () => {
            if ($(getSpotifyDOM(SpotifyDOM.AppRoot)).has(getSpotifyDOM(SpotifyDOM.AvatarMenu))) {
                ...
                lastOpen = Date.now();
                $(getSpotifyDOM(SpotifyDOM.AvatarMenu)).hide().fadeIn(200);
                ...
                this.addItem("SPDM Preferences", () => {
                    ...
                    $(baseModel).hide().fadeIn(200).appendTo('body > div:nth-child(7)');
    ...
    public addItem(name: string, onclick?: (dom: JQuery<this>) => void) {
        ...
        const menuItem = $(`<li role="presentation" class="b46bba08e80cdd2d0da8cca1e49c7440-scss"><a class="d2a8e42f26357f2d21c027f30d93fb64-scss d8298bd279d997e25cc981a71a6f28bf-scss" role="menuitem" tabindex="-1"><span class="ellipsis-one-line f3fc214b257ae2f1d43d4c594a94497f-scss" as="span" dir="auto">${name}</span></a></li>`).one('click', () => {
            $(getSpotifyDOM(SpotifyDOM.AvatarMenu)).fadeOut(200);
            $('#main > div > div.Root__top-container > div.Root__top-bar > header > button._3e75c7f07bdce28b37b45a5cd74ff371-scss').trigger('click');
            onclick($(this))
        });
        menuRoot.children().get(3).append(menuItem.get(0));
    }
